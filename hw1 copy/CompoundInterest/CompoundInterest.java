...
public class CompoundInterest {
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double value = perYear;
        for (int year = THIS_YEAR + 1; year <= targetYear; year +=1) {
            value += futureValue(perYear, rate, year);
        }
        return value;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double value = totalSavings(perYear, targetYear, rate);
        for (int i = 0; i < numYears(targetYear); i +=1) {
            value *= (1 - inflationRate*.01);
        }
        return value;
