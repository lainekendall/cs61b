...
class Move {
    ...
    static Move create(String s, Board board) {
        ...
        if (s.matches("[a-h][1-8]-[a-h][1-8]\\b.*")) {
            ...
            // System.out.println("string constructor null");
            return null;
        }
    }
    ...
    static Move create(int column0, int row0, int column1, int row1,
        ...
        if (!inBounds(column0, row0) || !inBounds(column1, row1)) {
            // System.out.println("create constructor:1 null");
            return null;
        }
        // new line!!!
        ...
        // maybe already done in Move private constructor?
        if (!isDirection(column0, row0, column1, row1)) {
            // System.out.println("create constructor:2 null");
            return null;
        }
        // end new line
        Piece movedPiece = board.get(column0, row0);
        if (movedPiece == EMP) {
            // System.out.println("moved piece is null");
            return null;
        }
    ...
    Direction getDirection() {
        int vertical = getRow1() - getRow0();
        int horizontal = getCol1() - getCol0();
        return direction(horizontal, vertical);
        
    }

    static boolean isDirection(int column0, int row0, int column1, int row1) {
        if ((!inBounds(column0, row0)) || (!inBounds(column1, row1))) {
            return false;
        }
        return (NOWHERE != direction(column1 - column0, row1 - row0));
    }

    static Direction direction(int column0, int row0, int column1, int row1) {
        if ((!inBounds(column0, row0)) || (!inBounds(column1, row1))) {
            return NOWHERE;
        }
        return direction(column1 - column0, row1 - row0);
    }

    static Direction direction(int horizontal, int vertical) {
        if (vertical < 0) { // S
            if (horizontal == 0) {
                return S;
